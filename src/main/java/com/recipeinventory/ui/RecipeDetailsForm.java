...
public class RecipeDetailsForm extends JFrame {
    ...
    public RecipeDetailsForm(int recipeId) {
        ...
        getRootPane().setBorder(Ui.pageBorder());
        details.setEditable(false);
        JButton rate = Ui.primaryButton("Rate & Review");
        JPanel card = Ui.card();
        card.add(Ui.scrollPane(details));
    ...
    private void load() {
        try {
            ...
            if (steps.isEmpty()) {
                text.append("(No steps added yet.)\n");
            } else {
                for (CookingStep step : steps) {
                    text.append(step.getStepNumber()).append(". ").append(step.getInstruction());
                    if (step.getStepType() != null && !step.getStepType().isBlank()) {
                        text.append(" [").append(step.getStepType()).append("]");
                    }
                    if (step.getTimeMinutes() > 0) {
                        text.append(" (").append(step.getTimeMinutes()).append(" min)");
                    }
                    text.append("\n");
                }
            }
